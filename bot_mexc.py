...
Versao Refinada: Filtro de Mercado Lateral + Pontos de Entrada/Saida (TP/SL)
"""
...
TIMEFRAME = "Min60"
INTERVALO_MIN_ALERTAS = 30
CONFIANCA_MINIMA = 65  # Aumentada para maior precisao
ADX_MINIMO = 25        # Filtro de tendencia (abaixo disso e mercado lateral)
# =============================================
...
class MexcMonitorBot:
    ...
    def __init__(self):
        ...
        print("🤖 Inicializando Bot MEXC Refinado...")
    ...
    def _calcular_indicadores(self, df: pd.DataFrame) -> Dict:
        ...
        # ATR (Average True Range) para Stop Loss e Take Profit
        tr1 = high - low
        ...
        # ADX (Average Directional Index) para evitar mercado lateral
        plus_dm = high.diff()
        minus_dm = low.diff()
        plus_dm[plus_dm < 0] = 0
        ...
        for i in range(1, len(high)):
            ...
            if trend == 1:
                if low.iloc[i] < sar_i:
                    trend, sar_i, ep, af = -1, ep, high.iloc[i], 0.02
                elif high.iloc[i] > ep:
                    ep, af = high.iloc[i], min(af + 0.02, 0.2)
            else:
                if high.iloc[i] > sar_i:
                    trend, sar_i, ep, af = 1, ep, low.iloc[i], 0.02
                elif low.iloc[i] < ep:
                    ep, af = low.iloc[i], min(af + 0.02, 0.2)
            sar.iloc[i] = sar_i
    ...
    def _analisar_sinal(self, ind: Dict) -> Tuple:
        if not ind or ind['adx'] < ADX_MINIMO:
            return None, 0, 0, 0, 0

        buy_pts, sell_pts = 0, 0
        if ind['price'] > ind['sar']: buy_pts += 2
        else: sell_pts += 2
        if ind['rsi'] < 35: buy_pts += 1
        elif ind['rsi'] > 65: sell_pts += 1
        ...
        if buy_pts > sell_pts:
            ...
            if confianca >= CONFIANCA_MINIMA:
                tp = price + (atr * 2)
                sl = price - (atr * 1.5)
                return "COMPRA", confianca, price, tp, sl
        elif sell_pts > buy_pts:
            confianca = (sell_pts / total) * 100
            if confianca >= CONFIANCA_MINIMA:
                tp = price - (atr * 2)
                sl = price + (atr * 1.5)
                return "VENDA", confianca, price, tp, sl
    ...
    async def _enviar_alerta(self, symbol: str, sinal: str, price: float, confianca: float, tp: float, sl: float, ind: Dict):
        ...
        msg = f"{emoji} **SINAL DETECTADO - {symbol}**\n"
        msg += f"📅 {datetime.now().strftime('%d/%m/%Y %H:%M')}\n\n"
        msg += f"🚀 **ENTRADA: ${price:.4f}**\n"
        msg += f"🎯 **TAKE PROFIT: ${tp:.4f}**\n"
        msg += f"🛑 **STOP LOSS: ${sl:.4f}**\n\n"
        msg += f"📊 Confianca: {confianca:.1f}%\n"
        msg += f"📈 Forca da Tendencia (ADX): {ind['adx']:.1f}\n"
        msg += f"🕒 Timeframe: {TIMEFRAME}\n\n"
        msg += "⚠️ *Filtro de mercado lateral ativo. Tendencia confirmada.*"

        try:
            await self.telegram.send_message(chat_id=TELEGRAM_CHAT_ID, text=msg, parse_mode='Markdown')
    ...
    def _atualizar_cache(self, symbol: str, novo_preco: float):
        ...
        tf_minutos = {'Min1': 1, 'Min5': 5, 'Min15': 15, 'Min30': 30, 'Min60': 60, 'H4': 240, 'D1': 1440}
        intervalo = tf_minutos.get(TIMEFRAME, 60)
        ...
        if ind:
            ...
            if sinal:
                asyncio.create_task(self._enviar_alerta(symbol, sinal, p, conf, tp, sl, ind))
